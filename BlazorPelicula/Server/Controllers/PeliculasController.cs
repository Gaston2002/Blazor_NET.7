...
namespace BlazorPelicula.Server.Controllers
{
    ...
    public class PeliculasController : ControllerBase
    {
        ...
        public async Task<ActionResult<HomePageDTO>> Get()
        {
            ...
            var peliculaEnCartelera = await context.Peliculas.Where(pelicula => pelicula.EnCartelera).Take(limite)
                .OrderByDescending(pelicula => pelicula.Lanzamiento).ToListAsync();

            var fechaActual = DateTime.Today;

            var proximoEstrenos = await context.Peliculas.Where(pelicula => pelicula.Lanzamiento > fechaActual)
                .OrderBy(pelicula => pelicula.Lanzamiento).Take(limite).ToListAsync();
        ...
        public async Task<ActionResult<List<Pelicula>>> Get(
            [FromQuery] ParamatrosBusquedaPeliculasDTO modelo)
        {
        ...
        public async Task<ActionResult> Put(Pelicula pelicula)
        {
            ...
            if (!string.IsNullOrWhiteSpace(pelicula.Poster))
            {
                ...
                peliculaDB.Poster = await almacenadorArchivos.EditarArchivo
                    (posterImagen, ".jpg", contenedor, peliculaDB.Poster!);
